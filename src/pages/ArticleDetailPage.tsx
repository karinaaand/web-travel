      ...
      <div className="container flex flex-col gap-6 sm:gap-7">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="space-y-2">
            ...
            <h1 className="text-3xl font-semibold tracking-tight text-slate-900">Detail artikel destinasi</h1>
          </div>
        ...
        {!articleQuery.isLoading && selectedArticle ? (
          <>
            <Card className="border-white/70 bg-white/92">
              <div
                className="min-h-[460px] bg-linear-to-br from-teal-900 via-teal-700 to-orange-300 bg-cover bg-center"
                style={cover ? { backgroundImage: `linear-gradient(180deg, rgba(7, 24, 29, 0.16), rgba(7, 24, 29, 0.82)), url(${cover})` } : undefined}
              >
                <div className="flex min-h-[460px] flex-col justify-end gap-6 p-6 text-white sm:p-8 lg:p-10">
                  ...
                  <div className="max-w-4xl space-y-4">
                    <h2 className="text-4xl font-semibold tracking-tight sm:text-5xl lg:text-[3.6rem] lg:leading-[1.02]">{selectedArticle.title}</h2>
                  ...
                  <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap">
                    <Button type="button" variant="secondary" onClick={() => void handleShare()} className="rounded-2xl">
              ...
              <Card className="border-white/70 bg-white/92">
                ...
                <CardContent className="space-y-5">
                  {contentBlocks.slice(0, 4).map((block, index) => (
                    <p key={index} className="whitespace-pre-wrap text-sm leading-8 text-slate-700 sm:text-[15px]">{block}</p>
                  ))}
                  {contentBlocks.length > 4 ? (
                    <details className="rounded-3xl border border-slate-200 bg-slate-50/70 p-5">
                      ...
                      <div className="mt-4 space-y-5">
                        {contentBlocks.slice(4).map((block, index) => (
                          <p key={`more-${index}`} className="whitespace-pre-wrap text-sm leading-8 text-slate-700 sm:text-[15px]">{block}</p>
                        ))}
                      </div>
                    </details>
                  ) : null}
